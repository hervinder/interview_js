class PriorityQueue {
  ...
  bubbleUp() {
    ...
    console.log("bubbleUp", this.values);
  }
  ...
  Dequeue() {
    ...
    console.log("dequue", this.values);
  }
}
...
function topKclosest(array, k) {
  ...
  console.log("this", priorityQueue.values);
}

function ClosestNumbers(array, k, x) {
  var priorityQueue = new PriorityQueue();
  for (let index = 0; index < array.length; index++) {
    const element = array[index];
    priorityQueue.Enqueue(element, Math.abs(x - element));
    if (priorityQueue.values.length > k) {
      priorityQueue.Dequeue();
    }
  }
...
function RearrangingString(string) {
  ...
  let str = "";
  let previousVisited = "#";
  while (repeatedString[0].priority > 0) {
    for (let index = 0; index < repeatedString.length; index++) {
      const element = repeatedString[index];
      if (previousVisited == element.val) {
        console.log("unmatched");
        repeatedString[0].priority = 0;
        break;
      }
      if (element.priority > 0) {
        str = str + element.val;
        element.priority--;
        previousVisited = element.val;
      }
    }
  }
...
// ClosestNumbers([2, 4, 5, 6, 9], 3, 6);

RearrangingString("aab");
